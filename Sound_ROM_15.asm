              ...
              ;
              ; pinball Sound ROM 15 (prev. erroneously referred to as Pinball ROM1)
              ;
              ;update based on Williams Sound ROM source code via :
...
;* TALKING EQUATES
;*
TALK     EQU  $EFFD    ;ENTRY TO TALKING  <--- ! this SPEECH ROM ADDR
TALKD    EQU  $EFFA    ;ENTRY FOR DIAGNOSTICS <-- and this one
;*
;* SCREAM EQUATES
;*
ECHOS    EQU  4
...
F812 : 86 37      ldaa  #$37     ;load A with value 37h (CB2 low, IRQ allowed)
F814 : A7 03      staa  $03,x    ;store A ijn addr X + 03h (0403 PIA1 CR port B)
F816 : 86 3C      ldaa  #$3C     ;load A with value 3Ch (CA2 set init high, no IRQs)
...
F82B : 48         asla           ;arith shift left A (bit0 is 0) x2
F82C : 48         asla           ;arith shift left A (bit0 is 0) x4
F82D : 48         asla           ;arith shift left A (bit0 is 0) x8
...
;*************************************;
;Variable Duty Cycle Square Wave Routine (SYNTH1)
;*************************************;
;VARI
...
;*************************************;
;Lightning+Appear Noise Routine (SYNTH2)
;*************************************;
;LITEN:
...
F8B1 : 76 00 05   ror  $0005     ;rotate right in addr 0005 (bit7 = C then C = bit0) (HI)
F8B4 : 76 00 06   ror  $0006     ;rotate right in addr 0006 (bit7 = C then C = bit0) (LO)
...
;LITE3:
F8BE : 4A         deca           ;decr A
F8BF : 26 FD      bne  LF8BE     ;branch if Z=0 SYN25 (LITE3)
F8C1 : 5A         decb           ;decr B (COUNT CYCLES)
F8C2 : 26 E5      bne  LF8A9     ;branch if Z=0 SYN23 (LITE1)
...
F8CA : 26 DB      bne  LF8A7     ;branch if Z=0 SYN22 (LITE0)
...
F8EC : 76 00 05   ror  $0005     ;rotate right in addr 0005 (bit7 = C then C = bit0) (HI)
F8EF : 76 00 06   ror  $0006     ;rotate right in addr 0006 (bit7 = C then C = bit0) (LO)
...
F945 : 76 00 05   ror  $0005     ;rotate right in addr 0005 (bit7 = C then C = bit0) (HI)
F948 : 76 00 06   ror  $0006     ;rotate right in addr 0006 (bit7 = C then C = bit0) (LO)
F94B : D6 0F      ldab  $0F      ;load B with value in addr 0F (FMAX)(SET FREQ)
F94D : 7D 00 15   tst  $0015     ;test if value in addr 0015=00 (N=0(MSB), Z=1) (DSFLG)
...
F95A : 22 12      bhi  LF96E     ;branch if C=0 and Z=0 SYN45 (FNOIS4)
;FNOIS3:
...
F96A : 23 F0      bls  LF95C     ;branch if C=1 and Z=1 SYN44 (FNOIS3)
...
F983 : 20 B9      bra  LF93E     ;branch always SYN42 (FNOIS1)
;FNOIS6:
...
;SCREM4:
FA14 : 74 00 0D   lsr  $000D     ;logic shift right (bit7=0) in addr 000D (TEMPA)(DECAY AMPLITUDE)
...
;ORGNT1:
FA4D : 7F 00 04   clr  $0004     ;clr (00) addr 04 (ORGFLG)
...
FA78 : 8D 0E      bsr  LFA88     ;branch sub SYNTH8 (ORGANL)
...
FA84 : 26 E8      bne  LFA6E     ;branch Z=0 PRM74 (ORGNT4)
;ORGNT5:
...
;*************************************;
;ORGANL
FA88 : CE 00 12   ldx  #$0012    ;load X with 0012h (#RDELAY)(scratch mem locations)
FA8B : 80 02      suba  #$02     ;A = A - 02h (0000 0010)
;LDLP:
FA8D : 23 15      bls  LFAA4     ;branch if lower or same SYN83 (LD1)
...
FAA8 : C6 FA      ldab  #$FA     ;load B with FAh (#ORGAN1!>8 MSB) <-- should be  >FA ?
FAAA : E7 01      stab  $01,x    ;store B in addr X + 01h
FAAC : C6 B2      ldab  #$B2     ;load B with B2h (#ORGAN1!.$FF LSB) <-- should be .$B2 ?
FAAE : E7 02      stab  $02,x    ;store B in addr X + 02h
;*************************************;
;Organ Routine (SYNTH8)
;*************************************;
;DUR=DURATION, OSCILLATOR MASK
;ORGAN
FAB0 : DE 0F      ldx  $0F       ;load X with addr 0F (DUR)
;ORGAN1 (stores 7E FAB2 jmp $FAB2)
...
FAD0 : 1B         aba            ;A = A + B <-- not in Pharoah/RatRace ROMs
...
FADC : 7E 00 12   jmp  L0012     ;jump extended addr 0012 (RDELAY)(scratch mem location?)
;ORGAN2:
FADF : 39         rts            ;return subroutine
;*************************************;
;Parameter Transfer (UTIL1)
;*************************************;
;TRANS
...
;*************************************;
;Background End Routine (PARAM10)
;*************************************;
;BGEND
...
;*************************************;
;Background Sound #2 increment (PARAM11)
;*************************************;
;BG2INC
...
FB26 : C1 1F      cmpb  #$1F     ;compare B with value 01h (#SP1MAX-1)
...
FB34 : 23 05      bls  LFB3B     ;branch if C and Z = 1 PRM123 (SP12)
...
;*************************************;
;GWAVE Loader (PARAM13X)
;*************************************;
;GWLD:
FB57 : 16         tab            ;transfer A to B (A unchanged) (MULKT BY 7)
...
FB84 : 2B 08      bmi  LFB8E     ;branch if N=1 (negative) PRM134 (GWLD3)(FINIS)
...
FBF2 : 09         dex            ;4 cycles
FBF3 : 01         nop            ;2 cycles
FBF4 : 01         nop            ;2 cycles (36 total)
FBF5 : 20 DF      bra  LFBD6     ;branch always SYN93 (GOUT)(SYNC 36)
;GEND
...
FC16 : 7D 00 12   tst  $0012     ;test (if 12 MSB is 1s) and set N,Z (GDFINC)
...
;GW2:
FC27 : 5D         tstb           ;test B and set N,Z (FOUND START YET?)
...
FC37 : 5D         tstb           ;test B and set N,Z (FOUND START?)
...
;WVDECA
FC5D : 4D         tsta           ;test A and set N,Z
...
FC76 : 54         lsrb           ;logic shift right B (bit7=0) (CALC 1/16TH)
FC77 : 08         inx            ;incr X
...
;WVDLP1:
FC7E : 10         sba            ;A = A - B (B unchanged) (DECAY)
...
;
;CHECK FOR PRESENCE OF TALKING PROGRAM (speech ROM?)
; some routine here not in this ROM code cmpb #$7E, jsr TALK
;
;IRQ1:
...
FCC9 : 48         asla           ;arith shift left A (bit0 is 0) (DOUBLE FOR ADDRESSING)
FCCA : CE FD 42   ldx  #$FD42    ;load X with value FD42h (#JMPTBL)(INDEX TO JUMP TABLE)(VWTAB)
FCCD : 8D 21      bsr  LFCF0     ;branch sub CALCOS (GET CORRECT INDEX)
...
; here is lines for cmpb #$7E, jsr TALKD (TALKING PRESENT)
...
;*************************************;
;GWVTAB - Wave table, 1st byte wavelength, given as decimal vals, diff lengths
;*************************************;
FE37 : 08 7F D9 FF D9 7F 24 00 24     ;GS2
;8,127,217,255,217,127,36,0,36 
;
FE40 : 08 00 40 80 00 FF 00 80 40     ;GSSQR2
;8,0,64,128,0,255,0,128,64
;
FE49 : 10 7F B0 D9 F5 FF F5 D9 B0     ;GS1
;16,127,176,217,245,255,245,217,176
FE52 : 7F 4E 24 09 00 09 24 4E        ;
;127,78,36,9,0,9,36,78
;
FE5A : 10 7F C5 EC E7 BF 8D 6D 6A     ;GS12
;16,127,197,236,231,191,141,109,106
FE63 : 7F 94 92 71 40 17 12 39        ;
;127,148,146,113,64,23,18,57
;
FE6B : 10 FF FF FF FF 00 00 00 00     ;GSQ22
;16,$FF,$FF,$FF,$FF,0,0,0,0
FE74 : FF FF FF FF 00 00 00 00        ;
;$FF,$FF,$FF,$FF,0,0,0,0
;
FE7C : 48 8A 95 A0 AB B5 BF C8 D1     ;GS72
;72,138,149,160,171,181,191,200,209 
FE85 : DA E1 E8 EE F3 F7 FB FD FE FF  ;
;218,225,232,238,243,247,251,253,254,255
FE8F : FE FD FB F7 F3 EE E8 E1 DA     ;
;254,253,251,247,243,238,232,225,218
FE98 : D1 C8 BF B5 AB A0 95 8A 7F     ;
;209,200,191,181,171,160,149,138,127
FEA1 : 75 6A 5F 54 4A 40 37 2E 25 1E 17 11 0C ;
;117,106,95,84,74,64,55,46,37,30,23,17,12
FEAE : 08 04 02 01 00
;8,4,2,1,0
FEB3 : 01 02 04 08 0C 11 17 1E 25 2E 37 40 4A 54;
;1,2,4,8,12,17,23,30,37,46,55,64,74,84
FEC1 : 5F 6A 75 7F 
;95,106,117,127
;
FEC6 : 10 59 7B 98 AC B3 AC 98 7B     ;GS1.7
;16,89,123,152,172,179,172,152,123
